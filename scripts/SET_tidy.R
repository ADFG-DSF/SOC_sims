...
SET_point %>%
  mutate(pct_lb = SET / lb_Ricker,
         pct_Smsy = SET / Smsy_Ricker) %>%
  pivot_longer(cols = c("pct_lb", "pct_Smsy"), 
               names_to = "Reference", 
...
plot_fit <- function(scenario_file, ...){
  rep_var = sample(unique(scenario_file$rep), 1, replace = FALSE)
  
  filters <- enquos(...)
  
  data <- 
    scenario_file %>%
    filter(!!!filters, rep == rep_var)
