...
There are two definitions embedded within the concern definitions that beg additional discussion. First, the definition of chronic inability is vague and had led to different interpretations at the division / regional level throughout the department.
...
The definition of a management concern (N \< escapement goal lower bound) makes it clear that we have exhausted our management capability. ADF&G should be transparent about this. In this instance a research plan is all that is necessary and appropriate. The goal of the research plan should be explicit: to improve stock assessments so that the state possesses the information necessary to determine an SET for the stock. Using the definition of an SET above this means we need to be paying for stock assessments robust enough to estimate returns from each spawning event. Returns per spawner of less than 1 from spawning events below the lower bound of the escapement goal would indicate the SET threshold had been crossed, thus defining the SET and putting ADF&G in a position to declare a conservation concern when necessary. Thus, SETs (and thus conservation concerns) would be determined on a case-by-case basis for individual stocks.

We should test our ability to reliably detects and SET threshold (and will, later) but for the moment let's define an SET as $1/2$ of the lower bound of the escapement goal and use these definitions in the simulation model described earlier to describe what our expectations might be with respect to SOC designations across a wide range of fisheries under both historic and current marine productivity regimes.
...
Under the assumption described above $\textrm{log}(\alpha^*)$ (stock productivity under low productivity scenarios , i.e. red lines in @fig-Ricker2) average about `r round(mean(dat_pctred$pct_red), 2)` of $\textrm{log}(\alpha)$ (stock productivity under historical productivity scenarios , i.e. black lines in @fig-Ricker2).

### Simulated Run Sizes with Historical and Low Productivity {#sec-simulated-run}
...
These results manifest themselves closely to what most of us might predict... SOC designations are rare under historical productivity regimes and become the expectation under low productivity regimes. One way to interpret these results might be to consider them in light of a hypothesis test where $H_0= \textrm{stock productivity is at it's historic average}$. In that light the risk of a false negative (declaring a stock as a stock of concern when productivity is near it's historic average) is negligible while the power of the test (ability to detect low productivity when it occurs is \~$50$% or less. If this interpretation is deemed valid it suggests satisfying a SOC definition in 4 of the last 5 years is a lagging indicator of an extreme reduction in stock productivity and ADF&G staff should be quick to make a SOC recommendation when a 4 of 5 threshold is met. It also suggests a mechanism to test other SOC designation criteria (x of y years, y consecutive years, rolling mean escapement) as perhaps more powerful indicators of low contemporary productivity.
