---
title: "Sustainable Salmon Fisheries Policy Stock of Concern Revision Recomendations"
author: "Adam Reimer"
format:
  html:
...
During the 2000-2001 board cycle, the board adopted into regulation the *Policy for the management of sustainable salmon fisheries* (SSFP; 5 AAC 39.222) and the *Policy for statewide salmon escapement goals* (EGP; 5 AAC 39.223). These two policies outline roles, responsibilities, principles and criteria to guide management and regulatory actions for salmon fisheries, including the designation of salmon stocks of concern (SOC).

Under commissioner directive, department staff undertook a review of the SSFP and EGP during the spring of 2025. The ADF&G escapement goal policy implementation team (EGPIT) was tasked with the review and created work groups to consider certain policy areas and report back to EGPIT with recommended changes. One of the major areas identified for review were the SOC provisions within the SSFP. A team of sixteen staff members met 12 times during the first half of 2025 to create recommendations which were reported back to EGPIT on two occasions. EGPIT approved of the recommended changes which were then presented back to the commissioner for final approval.

This document describes the staff recommendations. Staff identified both policy and technical issues during the review. Policy issues were addressed by changes to the structure and definitions of the SOC process as described int the SSFP and will be described in @sec-policy below. Technical issues refer to guidance ADF&G staff can use to implement the SOC provisions within the SSFP and are described in @sec-technical below.

# Policy Review {#sec-policy}

## Fisheries Management Review

The department manages salmon fisheries to attain escapement goals. Escapement goals are based on analyses of historical escapement, and incorporate information about historic production (the total number of fish that return from a given escapement) of the stock. Escapement goals are developed so that fishery managers can target escapements at levels that provide both escapement for future production and a surplus of salmon available for harvest. Because escapement goals are based on a time series of historical data the relationship they describe is relative to the "average" production of the stock.

Escapement goals are expressed as a range to allow for imprecision in fishery management actions and provide stable and predictable fisheries under normal and expected variability in average production. When escapements fall below the lower bound of the escapement goal, two possible causes can be identified. First, the stock may have been overfished by directed or incidental harvests. Often, overfishing is actionable by the department and/or board of fisheries and changes to fishery management plans can allow the stock to achieve the lower bound of the escapement goal over the next few seasons. Second, contemporary production may be less than the average production represented in the historical data. Variation in stock productivity can be moderate or severe. When a stock is experiencing moderate reductions in productivity, associated fisheries may require restrictions to time, area and/or gear which reduce harvest to ensure the escapement goal is achieved. Our ability to achieve escapement goals under moderate reductions in productivity will depend on the efficacy of these management actions. When a stock is experiencing extreme reductions in productivity complete fisheries closures may be required. After all fisheries have been closed, our management ability is exhausted and achieving the escapement goal depends on natural process which fishery managers cannot control.

## Description of the Existing SOC Process

When escapements are below the lower bound of the stock's escapement goal over a period of years the SSFP directs the board are department to consider whether the stock should be designated a stock of concern. The SSFP (5 AAC 39.222) describes the stock of concern process primarily in subsections (d)(1)(D)(ii), (d)(1)(D)(iii), (d)(3), (d)(4) and (d)(5). These subsections direct the department to recommend stock of concern listings to the board of fisheries at one of 3 levels: yield, management and conservation. Definitions of stock of concern and the different levels of concern are contained in 5 AAC 39.222(f):
...
Associated terms used in these definitions are defined in 5 AAC 39.222(F).

The board and department work together in a defined process to determine if a salmon stock meets the criteria as a stock of concern. As part of the escapement goal review that occurs for a regulatory area before a board meeting cycle, the department reviews escapements, harvests and other data relative to stock of concern designations. The department then presents a memo at the board Work Session to inform the board of any stocks within the regulatory area that appear to meet the criteria for stock of concern designation, the level of concern, and a recommendation that the board discuss this designation at their regulatory meeting. Upon request from the board the department will draft an action plan for the board to consider at their regulatory meeting. A draft action plan generally describes stock status, trends in escapement and harvest, and other factors that potentially impact stock production and/or harvest levels (e.g. changes in fish habitat, expanding and/or new fisheries). The draft plan also outlines regulatory options the department has, will, or could take for the board to consider while evaluating changes to the stock's management plan, and options may include options the department has, will, or could take to improve stock assessments and/or fish habitat. During the regulatory meeting the board of fisheries and department staff finalize the action plan by identifying actions the department should take to address the SOC listing.

After a stock is designated as a stock of concern and actions are implemented, the department reviews the status of the stocks of concern during their triennial review. If actions proved successful and the stock no longer meets the criteria for a stock of concern designation, the department will recommend delisting the stock: otherwise the department will recommend continued listing or a change in the level of concern.
...
A total of 48 stock of concern listings have occurred since 2000: 35 management concerns and 13 yield concerns (@tbl-listings). No stocks of conservation concern have been listed. Stock of concern listings have occurred throughout the state of Alaska; 11 in the Arctic-Yukon-Kuskokwim region, 19 in the Central Region, 13 in Southeast Alaska, and 5 in the Westward region. Across species, the 46 SOC listings have been applied to 25 Chinook stocks, 10 chum stocks, and 13 sockeye stocks. Twenty seven stock of concern listings have been delisted since the stock of concern policy was implemented in 2000.
...
#| echo: FALSE
#| label: tbl-listings
#| tbl-cap: "SOC listing history 2020-2025. This table shows each SOC listing made by the board of fisheries, some stocks are represented by several listings when the SOC type and/or the stock defintions changed while the stock remained in SOC status."
SOC_tab %>%
...
#| echo: FALSE
#| label: tbl-stocks
#| tbl-cap: "SOC stock history 2020-2025. In this table multiple listings for the same salmon stock are colapsed into a single entry provided the listings are temporally continuous."
multiple_listings <- 
  SOC_tab %>%
...
@fig-flowchart_current provides a flow chart describing the current SOC process. The first two "rows" of @fig-flowchart_current show decisions departmental staff must make before a recommendation to consider SOC status can be forwarded to the board of fisheries. The first row of the flowchart describes metrics that are used to evaluate whether a salmon stock qualifies as a stock of concern. One fundamental issue with the current process has to do with the wide variety of metrics used to indicate stock of concern status. Because each metric was developed to achieve a different management goal this diversity of metrics causes confusion about the goal of the stock of concern process itself. For example, stocks whose:
...
Notice that initiating the stock of concern process using a wide variety of metrics does not provide consistency with respect to the *type of risk* or whether it is the **fishery or stock** which is at risk. Most management metrics which are used to initiate the stock of concern process were designed to address fishery issues rather than conservation issues. This lies in stark contrast to the public perception of the stock of concern process which assumes stock sustainability is at risk when a stock is listed as a stock of concern.

The second row of the flowchart identifies the 3 levels of concern identified in the SSFP. The conservation concern level is cleanly defined in that stocks identified as a conservation concern clearly indicate a risk to stock sustainability. That said, a conservation concern has never been recommended by the department due to the lack of a standardized procedure for identifying an SET. See @sec-SET for an SET procedure. Notice that conservation concerns are a subset of management concerns given the definition of the SET 5 AAC 39.222(f)(39) which states that "the SET is lower than the lower bound of the BEG and lower than the lower bound of the SEG".

In contrast to the conservation concern definition the definitions for yield and management concerns primarily indicate risk to current and future fisheries. In theory, management concerns and yield concerns are clearly differentiated because escapements for stocks of management concern are chronically falling outside the bounds of the escapement goal while escapements for stocks of yield concern are falling within the bounds of the escapement goal but are associated with run sizes close enough to the lower bound of the escapement goal that fisheries are closed or severely restricted. Yield concerns were frequently used when the SOC process was first implemented. During the first 10 years of the SOC process, 2000-2011, SOC listings were evenly split between stocks of yield concern (13) and stocks of management concern (13). Over time staff realized differentiating between stocks of yield and management concern was difficult to do objectively because stocks chronically failing to realize yields are necessarily experiencing low productivity and associated with run sizes at or near the lower bound of the stock's escapement goal. In addition, because department staff manage fisheries to achieve escapement goals, not a specified level of harvest, the yield realized during small runs is largely a function of the annual production and cannot be increased by fisheries management. This makes identifying a target yield a subjective exercise which may be either unachievable or in conflict with the department's responsibility for escapement goal management. Given these issues the department has not recommended a new stock of yield concern since 2011.

The third "row" of @fig-flowchart_current describes the department and board response after the department has recommended the board consider a stock of concern designation, i.e. the action plan. One way to evaluate if action plans produced by the board and department have resulted in stocks being removed from the SOC process is to look at past stock of concern listings and delisitings. This question is poorly answered by @tbl-listings because several stocks have been listed and delisted contiguously, most commonly by changing the level of concern associated with the stock, while remaining in the SOC process. In @tbl-stocks salmon stocks which have been considered stocks of concern are shown as unique entries each time they entered the SOC process. When looked at this way the 48 SOC listings that have occurred since 2000 have been associated with 39 unique salmon stocks (21 Chinook, 9 chum and 9 sockeye) and 41 entries into the SOC process. For stocks that have been listed at least 3 years (and thus had an opportunity to be delisted under the triennial BOF cycle) we see that while 88% and 86% of the chum and sockeye salmon stocks, respectively, that have entered the stock of concern process were ultimately removed from the process while only 36% of Chinook salmon stocks have done so. For the stocks which were removed from the SOC process, 83% of Chinook salmon, 71% of chum salmon and 71% of sockeye salmon exited the SOC process within 2 board cycles of being listed. Our team interpreted this information as an indication that action plans are effective in some situations and ineffective in others.

Which stock and fishery issues are effectively resolved by action plans may be illuminated by considering the cause of the stock of concern listing. When escapement is chronically below the lower bound of escapement goal due to overharvest, action plans have a reasonable likelihood of success within 1-2 board cycles (3-6 years). In contrast, when escapement is chronically below the lower bound of escapement goal due to severely reduced productivity the department probably exhausted most reasonable management measures through it's emergency order authority in an effort to achieve the escapement goal before the goal was missed. In this instance actions described in the action plan are likely redundant and may have already demonstrated that they did not reduced harvest enough to achieve the escapement goal and resolve the underling issue.

::: panel-tabset
### Current SOC Process Flowchart
...
knitr::include_graphics("./flowchart_current.jpg")
```

### Proposed SOC Process Flowchart
...
knitr::include_graphics("./flowchart_proposed.png")
```
:::

## Policy Recommendations

ADF&G review of the existing SOC process identified confusion resulting from housing a wide variety of fishery management objectives within the same process, questionable utility behind the 3 different stock of concern levels, and questionable efficacy of action plans in at least some situations where the SOC process is currently applied. The goal of our suggested policy revisions is to correct these issues. The major feature of the revised policy is the separation of low annual abundance issues into two distinct processes according to the availability of a harvestable surplus; a revised stock on concern process whose scope is narrowed to situations where harvestable surpluses are absent and a fishery review process for situations where harvestable surpluses are available.

### SOC Process Revisions

The stock of concern process is narrowed to situations where harvestable surpluses are absent by evaluating chronic inability of the stock's annual run size (rather than annual escapement) relative the the lower bound of the SEG or BEG. When annual run sizes are below the lower bound of the stock's escapement goal it means that there was no management action which would have allowed the lower bound of the escapement goal to be achieved. Therefore, this change reduces the SOC process to only stocks which are experiencing extreme reductions in productivity. This definition also clarifies that SOC designations are only relative the the lower bounds of the escapement goal range and should be made relative to the SEG or BEG set by the department.

Several other changes to the existing SOC are recommended to address issues identified during the review process. We recommend the department designate (rather than recommend) stocks of concern after determining they meet the SOC criteria. Evaluation for SOC eligibility would occur as part each management area's triennial review and be made at the work session preceding each management area's regular board meeting. Allowing the department to make SOC designations directly ris more timely and less confusing than the current process (where SOC designations are not made until the BOF meeting) and consistent with the department's existing responsibilities to to establish SEGs and BEGs directly.

After a stock is designated as a stock of concern we recommend the SET for the stock be identified and communicated the the board and public. Doing so immediately after the stock is designated as a stock of concern clearly communicates the risk to the stock. If annual run sizes are below the lower bound of the stock's SEG or BEG but above the stock's SET then the stock is experiencing production declines that are so severe the stock can no longer support fisheries. We call this situation a productivity risk. If annual run sizes are below the stock's SET then the stock is experiencing production declines that are so severe that stock sustainability may be compromised. We call this situation a depletion risk.

Finally, we recommend SOC designations are addressed by the board and department collaborating to create a recovery plan. A recovery plan is a temporary management plan which supersedes the stock's existing management plan and sunsets when the stock is removed from stock of concern status. Replacing action plans with recovery plans in the revised process clearly communicates that stocks of concern are caused by environmental and biological variation in productivity which exceeds our management capability and will require significant, prolonged reductions to harvest pressure until productivity returns to a more normal state.
...
While department staff felt the SOC process would be improved by restricting it to severe productivity reductions staff also recognized that the existing process has be effective in resolving some fishery issues faced by the board and department. To preserve this utility we recommend the SOC process by augmented by a parallel process which would be enacted at the discretion of the board and appropriate for less severe reduced productivity anomolies and/or overharvest situations. The primary characteristic of this process is that it allows for broad fishery issues to be brought before the board without a public proposal.

Because fishery reviews occur at the board's discretion the department's role in initiating the process is limited to notification of potential issues. Notification occurs at the board work session preceding each management area's regular board meeting. There are two situations where the department is obligated to notify the board. First, the department will notify the board whenever a escapements have displayed a chronic inability to achieve the lower end of a board designated OEG or inriver run goal. Second, the department will notify the board whenever escapements have displayed a chronic inability to achieve the lower end of the stocks SEG or BEG and the stock does not qualify as a SOC[^1]. After notification the board discusses these stocks and any others where they feel a fishery review might be warranted and directs the department to conduct the review prior to the regular board meeting for each selected stock. It is important to note that the board has complete discretion over which stocks are selected for review.

[^1]: i.e. when run size was sufficient to exceed lower end of the goal range.

At the regular board meeting for each selected stock the board uses the information in the department provided fishery review and any information associated with proposals submitted for that stock to identify potential management changes. Regulatory changes associated with fishery review occur through the regular board process and may consist of maintaining the status quo, creating new or modifying existing management plans, and action on any submitted proposals.

The right hand column of @fig-flowchart_proposed shows a flowchart describing the recommended fishery review process.

### Data Limited Fisheries

The distinction between the SOC and the fishery review processes described above rests on whether there is a harvestable surplus available to allocate: no for the SOC process and yes for the fishery review process. While this distinction is easy to make when stock specific harvest information is available there are many salmon stocks where we lack this information and cannot make a purely quantitative distinction. In those cases it will be up department staff to make a subjective decision regarding the likelihood that mixed stock fishery harvest for the stock in question exceeds the difference between the annual escapement and the lower bound of the stock's escapement goal. When staff believe the mixed stock fishery harvest for the stock does not exceed the difference between the annual escapement and the lower bound of the stock's escapement goal the stock would enter the SOC process. When staff believe the mixed stock fishery harvest for the stock does exceeds the difference between the annual escapement and the lower bound of the stock's escapement goal the stock would enter the fishery review process.
...
rep_scenarios_gp %>%
  ...
  geom_rect(aes(xmin = lb,
            ...
            fill = "grey95",
            inherit.aes = FALSE) +
  geom_line() +      geom_line(aes(y = R_2), linetype = 2) +
  geom_abline() +
...
@fig-S shows simulated escapement time series for populations parameterized as described above. Notice that the escapement time series is unresponsive to the modeled change in productivity immediately after the reduced productivity regime begins. We also see that while escapements for populations with non-depensatory dynamics ($\gamma=1$) are asymptotic with the lower bound of the escapement goal, abundance for populations with depensatory dynamics decreases slowly under the simulation assumptions. Thus the range of simulations considered include both our traditional assumptions (non-depensatory dynamics) and behavior precautionary management should avoid[^7]. I'd argue these simulation also suggest targeting too small of a percentage of MSY is dangerous as discovery of potential depensatory dynamics is delayed until spawning abundances are already dangerously low.
...
Stock of concern (SOC) listing and de-listing criteria were identified by staff and leadership as an issue that should be considered during EGPIT's review of the SSFP. The SSFP provides guidance on SOC listing and de-listing criteria relative the the definition of Chronic Inability which is defined in 5 AAC 39.222(f):

::: {#nte-CI .callout-note appearance="simple" icon="false"}
...
We view this topic as technical because while the definition itself appears to be workable, interpretation by ADF&G staff have differed across division, regions and species. In general, ADF&G staff have considered missing an escapement goal for 3, 4, or 5 of the last 5 years as sufficiently chronic to recommend listing a stock as a stock of concern. De-listing criteria are also not standardized but I'll suggest the inverse of the listing criteria (making the escapement goal 3, 4, or 5 of the last 5 years while listed as a SOC) seems to be the default. In what follows I'll assume the most common criteria Department staff have used is recommending a stock is listed as a SOC listing when it has missed it's escapement goal for 4 of the last 5 years and recommending a stock is de-listed as a SOC when it has made it's escapement goal for 4 of the past 5 years.
...
Complete expansion of these parameter combinations resulted in 54 different scenarios of non-depensatory spawner-recruit dynamics. There were 162 total scenarios of spawner-recruit dynamics after consideration of the depensatory dynamic spawner-recruit scenarios and 324 scenarios after consideration of 2 different percentages of MSY at the lower bound of the escapement goal (@tbl-ci_scenarios).

::: panel-tabset
### Chronic Inability Scenarios

```{r}
#| echo: FALSE  
#| label: tbl-ci_scenarios  
#| tbl-cap: "Spawner-recruit and fishery parameter combinations used to evaluate chronic inability in this simulation study."
rep_scenarios %>%
  group_by(scenario) %>%
  filter(rep == min(rep)) %>%
  mutate(age_mat_char = gsub("c|`| ", "", as.character(age_mat))) %>%
  mutate(Rmax = round(exp(lnalpha_1)/beta*exp(-1), 0),           
         Smax = 1/ beta) %>%
  select(scenario, lnalpha_1, sigma, phi, gamma, pct_MSY, age_mat_char, Rmax, Smax) %>%
...
```{r}
#| echo: FALSE  
#| label: tbl-SET_scenarios  
#| tbl-cap: "Spawner-recruit and fishery parameter combinations used to evaluate SETs in this simulation study."
rep_scenarios_gp %>%
  group_by(scenario) %>%
  filter(rep == min(rep)) %>%
  mutate(Rmax = round(exp(lnalpha_1)/beta*exp(-1), 0),           
         Smax = 1/ beta) %>%
  select(scenario, lnalpha_1, sigma, gamma, pct_MSY, Rmax, Smax) %>%
...
Both the current and revised stock of concern process requires a sustainable escapement threshold (SET) to be determined before a stock can be identified as a conservation concern (see @nte-levels). In contrast to a escapement goal, where managers are compelled to distribute escapements within the escapement goal range, a SET represents a limit reference point that managers should strive to remain above. The SET is defined in 5 AAC 39.222(f) as:

::: {#nte-SET .callout-note appearance="simple" icon="false"}
### SET
...
In this document we will explore using the peak of the $\textrm{log}\left(\frac{R}{S}\right)$ curve as an approximation for the SET (see @fig-gamma, bottom panel) . The peak of the curve can be found using calculus and is located at $\frac{(\gamma-1)}{\beta}$ when $\gamma \ge 1$. One primary consideration would be how this point relates to other reference points used in fisheries management. For example, the SET definition make specific reference to the lower bound of the stock's escapement goal.

@fig-setlb1 shows the relationship between the peak of the $\textrm{log}\left(\frac{R}{S}\right)$ curve and the the lower bound of possible escapement goals for a single stock with varied levels of depensatory dynamics. Notice that when depensatory dynamics are included in the spawner-recruit model the range of escapements that provide x% of MSY narrows and shifts towards higher escapements. Since ADF&G currently assumes non-depensatory dynamics for all salmon stocks, managers would be using the goal ranges shown in the top row of @fig-setlb1 prior to a SOC listing. One critical realization is that if stock assessments are updated to allow for possible depensatory spawner-recruit dynamics in association with SOC listings then changes to escapement goal bounds are likely to be implied by the new assessment. Also note that while the SETs shown are smaller than the lower bounds of escapement goals that provide 90% of MSY, SETs can exceed the lower bounds of escapement goals that provide only 50% of MSY when depensatory dynamics are strong.
